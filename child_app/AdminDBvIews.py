...
def add_medical_history_save(request):
    if request.method != "POST":
        ...
        curr = connection.cursor()
        try:
            curr.execute("INSERT INTO child_app_medical_history VALUES (%s, %s, %s, %s, %s)",
                         [mno, datech, mtype, rlink, cid])
...
def add_donation_history_save(request):
    if request.method != "POST":
        ...
        curr = connection.cursor()
        try:
            curr.execute("INSERT INTO child_app_donation_history VALUES (%s, %s, %s, %s, %s)",
                         [Don_id, DName, DPanno, Ddate, Amt])
...
def edit_child_save(request):
    if request.method != "POST":
        ...
        try:
            curr.execute(
                "UPDATE child_app_child SET First_Name = %s,Last_Name = %s,DOB = %s,DOA = %s,CPhoto = %s,CANo = %s,CPANo = %s,GName = %s,GANo = %s,GPANo = %s,rid_id = %s  WHERE child_id = %s",
                [First_Name, Last_Name, DOB, DOA, CPhoto, CANo, CPANo, GName, GANo, GPANo, rid_id, Child_id])
...
def edit_room_save(request):
    if request.method != "POST":
        ...
        curr = connection.cursor()
        try:
            curr.execute("UPDATE child_app_room SET floor = %s, max_occupancy = %s WHERE room_id = %s",
                         [floor, max_occupancy, room_id])
...
def edit_medical_history_save(request):
    if request.method != "POST":
        ...
        try:
            curr.execute(
                "UPDATE child_app_medical_history SET datech = %s,rlink = %s,mtype =%s, cid_id =%s WHERE mno = %s",
                [datech, rlink, mtype, cid_id, mno])
...
def edit_office_bearers_save(request):
    if request.method != "POST":
        ...
        curr = connection.cursor()
        try:
            curr.execute("UPDATE child_app_office_bearers SET position = %s, eid_id=%s WHERE chair_no = %s",
                         [position, eid_id, chair_no])
...
def edit_donation_history_save(request):
    if request.method != "POST":
        ...
        try:
            curr.execute(
                "UPDATE child_app_donation_history SET DName = %s, DPanno = %s,Ddate = %s,Amt = %s WHERE Don_id = %s",
                [DName, DPanno, Ddate, Amt, Don_id])
